...
const getRand = <T>(arr: readonly T[]): T => arr[Math.floor(Math.random() * arr.length)];

export const generateName = (): string => `${getRand(ADJECTIVES)} ${getRand(NOUNS)} ${getRand(EVENT_TYPES)}`;
